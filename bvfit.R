...
function(x, start, ..., sym = FALSE, nsloc1 = NULL, nsloc2 = NULL, cshape = cscale, cscale = cloc, cloc = FALSE, std.err = TRUE, corr = FALSE, method = "BFGS", warn.inf = TRUE)
{
                       ...
                       dep)
    {
        ...
        if(any(c(scale1,scale2) < 0.01) || dep < 0.2 || dep > 10 || shape1 < -0.5 ||
           shape1 > 0.5 || shape2 < -0.5 || shape2 > 0.5)
            return(1e6)
        if(!is.null(nsloc1)) {
            ns <- numeric(length(loc.param1))
            for(i in 1:length(ns))
        ...
        if(any(c(m1l,m2l,bvl) == 1e6)) return(1e6)
        else if(cshape) return(m1l + m2l + bvl - dbeta(x = shape1+0.5, 1, 4, log = TRUE))
        else return(m1l + m2l + bvl - dbeta(x = shape1+0.5, 1, 4, log = TRUE) 
                    - dbeta(x = shape2+0.5, 1, 4, log = TRUE))
